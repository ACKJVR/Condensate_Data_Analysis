...
class wetting_model(Model):
    ...
    def rescale_data(self,pressure_data):
        super().rescale_data()
        length_data = self.seg.x_data
        time_data = self.seg.time_data
        length_data = np.abs(length_data**2 / self.L0**2 - 1)
        pressure_slope = pressure_data[0]
        pressure_int = pressure_data[1]
        time_data = np.abs((self.pressure*pressure_slope + pressure_int)*(time_data-self.t0)/self.L0**2)
